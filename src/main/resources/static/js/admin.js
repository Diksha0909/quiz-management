...
async function loadQuizzes(){const res=await fetch(api+'/admin/quizzes'); const data=await res.json(); const tb=document.getElementById('quiz-table'); tb.innerHTML='';
 data.forEach(q=>{const tr=document.createElement('tr'); tr.innerHTML=`<td>${q.id}</td><td>${q.title}</td><td><a href='/questions.html?quizId=${q.id}'>Open</a> <button onclick='delQuiz(${q.id})'>Delete</button></td>`; tb.appendChild(tr);});}
